...
Deployed: Worker version `ccb4b471-2a0d-4602-9e10-b7f8c321dddb` on 2026-08-28; schema compatibility verified against all 44 fields/verbs. Live dogfooding correctly rejected an unlisted `codex` actor; add that actor to the git hook credential's `allowed_actors` in `RETRACE_CREDENTIALS` before Codex-authored commits can be recorded.
