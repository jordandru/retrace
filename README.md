...
3. Attribution — `retrace doctor` flags a HEAD stored as human that carries live-hook agent evidence (`location.session` / `surface=agent`); `--gate` fails it (trailer-omit used to skip instruct-root). Still open: distinguish content author, committer, relayer, and credential principal; pin / session mismatch across MCP vs git.
