...
2. Seal what the server actually knows — `received_at` is stamped then dropped from the hash (`chain.ts` `hashPayload`), so a caller `timestamp` can be backdated undetectably. `caused_by` is checked at write (exists, same project). Adapter idempotency prefixes (`git:` / `gd:` / `gh:`) are reserved at write so a caller key cannot shadow the git hook / Drive / GitHub mapper.
...
Deployed: Worker version `3f86e8d0-528e-4c48-a3e8-35095f636200` on 2026-08-30 (`b4fe762` — export coverage + Ed25519 JWK `alg` strip; live full export VALID 807/807). Claude Code, Codex, Gemini CLI, Grok, and GitHub Copilot now have separate scoped identities, and the git-hook credential allows commits from all five.
