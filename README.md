...
**Now:** niche 5 first slice is on `main` (`8db31ff`) — Drive ingest can carry `caused_by`. Live check: set Apps Script `RETRACE_CAUSED_BY`, edit the watched-folder Doc, `testOnce`, confirm the new `gdoc:` event is linked, then clear the property. Do not re-run `setup`. Event #594 stays a root unless attested later.

**Next (earn “verifiable”, not more connectors):**
1. Completeness — pagination / export truncation, capture windows, reconcile Git / GitHub / Drive vs the ledger. A hash chain does not prove no event was omitted.
2. Attribution — distinguish content author, committer, relayer, and credential principal. Trailer-omit still looks human and bypasses the gate. Drive `RETRACE_CAUSED_BY` is still a manual property (stale-context).
3. Local `retrace-serve` — default-closed auth and bind host.
4. Dogfood — boxing-rpg coverage is the honest metric; a second project above 95% beats another integration.

**Not next:** C2PA / content credentials, LangSmith, an IdP, TRACE/TEE, a Cursor credential, more adapters, batch-amending the ledger.
...
Deployed: Worker version `7f7058d1-7de8-49cf-91e1-84dc6191847e` on 2026-08-29 (Drive `caused_by` mapper). Claude Code, Codex, Gemini CLI, Grok, and GitHub Copilot now have separate scoped identities, and the git-hook credential allows commits from all five.
