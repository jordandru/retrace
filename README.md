...
1. Completeness — `verifyExportBundle` reports omission for full exports (count, contiguous seq, head hash; scoped bundles say “not checkable”), and `retrace-export checkpoint` + `verify --checkpoint` pin the head in a git-committed `.retrace/checkpoints.jsonl` so removal after export is detectable. Still open: checkpoint on a schedule (CI) or a transparency log, pagination / export truncation, capture windows, and reconciling Git / GitHub / Drive vs the ledger.
2. Seal what the server actually knows — `received_at` is stamped then dropped from the hash (`chain.ts` `hashPayload`), so a caller `timestamp` can be backdated undetectably. `caused_by` is checked at write (exists, older, same project); a dangling parent is 400, not stored as a root. Sealed amendments that fail those rules are counted as `ineffective_amendments` in status instead of being skipped. Adapter idempotency prefixes (`git:` / `gd:` / `gh:`) are reserved at write so a caller key cannot shadow the git hook / Drive / GitHub mapper.
...
Deployed: Worker version `b2cecb7a-0c6e-424a-a27e-d057accc148f` on 2026-08-30 (`e6cb5d1` — `ineffective_amendments` in /status, live count 1; prior `3f86e8d0` signed full export VALID 807/807). Claude Code, Codex, Gemini CLI, Grok, and GitHub Copilot now have separate scoped identities, and the git-hook credential allows commits from all five.
