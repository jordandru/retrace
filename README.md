...
npm exec --package=@retrace/cli -- retrace doctor                             # read-only preflight; exits nonzero when capture is not ready
npm exec --package=@retrace/cli -- retrace status                             # human-readable integrity + capture + causality status
...
Deployed: Worker version `649243a7-a304-4bfb-b3db-f99447f6611d` on 2026-08-28; schema compatibility verified against all 44 fields/verbs. Claude Code, Codex, and Gemini CLI now have separate scoped identities, and the git-hook credential allows commits from all three agents.
