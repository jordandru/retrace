...
Deployed: Worker version `5f71b0ca-6bea-4ec1-bc79-b1be35911756` on 2026-08-31 (`2acdcca` — fail-closed 503 when unconfigured, share caching/rate-limit, LIKE/limit binding; prior `30c472e3` with `5f8c881` router hardening and `hash_v: 2` seals, `3d8edca8` sealed_by stamp). Claude Code, Codex, Gemini CLI, Grok, and GitHub Copilot now have separate scoped identities, and the git-hook credential allows commits from all five. This checkout also pins Cursor Agent (`cursor-agent`) the same way; `retrace-admin new-team` still does not mint it.
