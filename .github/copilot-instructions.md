...
- Add commit trailers `Retrace-Actor: github-copilot`, `Retrace-Model: <actual model>`, and `Retrace-Caused-By: <instruction event id>`.
- These instructions are authoritative for Copilot identity. Do not copy another agent's `Retrace-Actor` value from `CLAUDE.md`, `GEMINI.md`, or `GROK.md`.
