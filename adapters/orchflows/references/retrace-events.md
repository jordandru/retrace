...
- Server stamps (`sealed_by`) and producer signatures are whatever the host's Retrace credential provides. A generic installation has server-stamped, unsigned events; the ledger event itself carries `sealed_by`, which a reader can inspect. The library does not add a signature.
