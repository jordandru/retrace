...
export function mapGithubWebhook(event: string, payload: any, opts: GithubMapOptions = {}): EventInput[] {
  ...
  if (event === "push" && opts.includePush) {
    return (payload.commits ?? []).map((c: any, i: number) => {
      ...
      const parents: string[] = Array.isArray(c.parents)
        ? c.parents.filter((p: unknown): p is string => typeof p === "string")
        : i === 0
          ? (typeof payload.before === "string" && payload.before && !/^0+$/.test(payload.before) ? [payload.before] : [])
          : [String(payload.commits[i - 1].id)];
      return {
        ...
        method: { tool: "git", automated: r.actor.type !== "human", params: { sha: c.id, producer: "github-push", ref: payload.ref, pusher: payload.pusher?.name, raw_message: String(c.message ?? ""), author: { name: c.author?.name, email: c.author?.email ?? (c.author?.username ? `github:${c.author.username}` : undefined) }, parents } },
