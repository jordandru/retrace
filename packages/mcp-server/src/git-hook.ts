...
import { EventInput, appendEvent, describeEvent, Event, PRODUCER_SIG_FORMAT_V2, resolveCommitActor, classifyCommitClaim, attachClaimDecision, ClassificationUnavailableError, parseTrailerPolicy, isGitCommitSeal, isLegacyClientCommitSeal } from "@retrace-dev/core";
...
async function logCommit(repo: string, sha: string, cfg: Cfg, live = false): Promise<{ event: Event; deduped: boolean }> {
  ...
  if (!(store instanceof RemoteStore) && parseTrailerPolicy(process.env.RETRACE_TRAILER_POLICY) === "shadow" && isGitCommitSeal(input) && !isLegacyClientCommitSeal(input)) {
    const classified = await classifyCommitClaim({ store, input, producer: "git-hook", sealedBy: "unstamped", trailerPolicy: "shadow" });
