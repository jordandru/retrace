...
export async function amendAttributionMain(flags: Record<string,string|boolean>): Promise<number> {
  ...
  const preview=()=>({recorded:false,advisory:true,head:options.snapshot!.head,policy_digest:options.context!.policy_digest,git_facts_digest:options.context!.git_facts_digest,result:preflight.result,affected:preflight.affected,input:preflight.input});
