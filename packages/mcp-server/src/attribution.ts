...
export async function amendAttributionMain(flags: Record<string,string|boolean>): Promise<number> {
  ...
  const read=async()=>store instanceof RemoteStore ? (await fetchVerifiedRemoteEvents(store,project,flags.pubkey,undefined,producerVerifyOptsForRepo(repo))).events : await store.all(project);
...
export async function attributionViewForStore(store: ReturnType<typeof makeStore>, repo: string, project: string) {
  try {
    const events = store instanceof RemoteStore ? (await fetchVerifiedRemoteEvents(store, project, undefined, undefined, producerVerifyOptsForRepo(repo))).events : await store.all(project);
