...
- `actor.model` verbatim as the runtime reports it. If the runtime exposes no identifier, omit the
  field rather than guess — and omit the `Retrace-Model` trailer for the same reason: earlier sessions
  wrote `gpt-5` under newer models, and omission is the honest fallback (agent-rules 4 and 6).
