# Retrace — full reference

Everything that used to be the README, moved here verbatim on 2026-09-04 so the
[README](../README.md) can stay short. Section anchors are unchanged. New readers should
start with [Retrace by example](examples.md); the [SETUP-GUIDE](../SETUP-GUIDE.md) is the
guided walkthrough (clone → MCP → Worker → GitHub/Drive).
...
Same block works in Claude Desktop's `claude_desktop_config.json`. Other harnesses keep the same `command` / `args` / `env` with their own actor and config file: Gemini CLI (`RETRACE_ACTOR=gemini`, `.gemini/settings.json`), Grok Build TUI (`grok`, `~/.grok/config.toml`), Codex (`codex`, plus `AGENTS.md`), GitHub Copilot CLI (`github-copilot`, `~/.copilot/mcp-config.json`), VS Code Copilot Chat (`github-copilot`, VS Code **user** MCP — `MCP: Open User Configuration`; do not commit `.vscode/mcp.json`), Cursor Agent (`cursor-agent`, `.cursor/mcp.json` with the token in `~/.retrace/cursor.env` via `envFile`), OpenCode (`opencode`, committed `opencode.json` plus `OPENCODE.md`; token in `~/.retrace/opencode.env` via the bash `-c` env-file wrapper). Each client needs its own pinned `RETRACE_TOKEN` — never Claude’s — or the Worker will seal events as the wrong actor. For the live Worker, add `RETRACE_URL` and that scoped token; locally, events go to `~/.retrace/retrace.db` (override with `RETRACE_DB`). `retrace-admin` still mints the original five harnesses only (`add-agent --harness opencode` is opt-in). GitHub.com Copilot coding agent cannot reach this checkout's stdio MCP.
...
Tip for agents (put in `AGENTS.md` / `CLAUDE.md` / `GEMINI.md` / `GROK.md` / `OPENCODE.md` / `.cursor/rules/retrace-provenance.mdc`): *when committing, add trailers `Retrace-Actor: <your harness id>`, `Retrace-Model: <model>`, and `Retrace-Caused-By: <instruction event id>`.* Do not copy another harness's actor id. OpenCode also auto-loads `AGENTS.md`; `OPENCODE.md` must override Codex's actor id.
...
Full exports are **cron-precomputed**: the hourly Worker cron builds and signs each opted-in project's bundle once per moved head and stores it (D1, chunked), and `GET /projects/:p/export` serves those exact bytes — O(1) CPU per request, which is what keeps a large ledger's export from hitting the Worker CPU limit (the 503/error-1102 wall found 2026-09-03 at ~1.5k events). A response carries `x-retrace-export-cache: hit` or `stale` — a stale bundle (head moved since the last cron pass, at most an hour) is still a complete signed export against its own claim, with `x-retrace-export-cached-head`/`-live-head` naming the gap; `?fresh=1` forces a live rebuild. Scoped exports (`--artifact` etc.) always build live.
...
| `retrace_history` | newest-window timeline (`truncated` / `before_seq` to walk older pages); filter by artifact / actor / action / time / text |
...
The Worker can expose a compatibility-first Streamable HTTP endpoint at `/mcp`. It is **off unless `RETRACE_MCP_ENABLED=1`**. This is an audit surface, not a remote administration surface: it exposes exactly `retrace_log`, `retrace_instruct`, `retrace_history`, `retrace_why`, `retrace_status`, `retrace_verify`, `retrace_projects`, `retrace_lineage`, and an in-memory-only `retrace_export`. It does not expose `retrace_amend`, `retrace_share`, or any file-writing option.

Remote MCP accepts only a bearer token belonging to a `trust: "pinned"`, `actor.type: "agent"` credential with exactly one allowed project. Owner tokens, query-string tokens, assert credentials, and multi-project credentials are rejected. The Worker pins every read/write to that project and stamps the credential's actor. For this pilot the actor is `openclaw`; **NemoClaw is the sandbox/runtime, not another actor**. The endpoint does not claim a producer signature—the compatibility result is server-stamped provenance, with a producer-signing adapter deferred until the integration is stable.

Add OpenClaw to an existing team without changing `new-team`'s original five-harness default or redistributing old secrets:
...
REST API: `POST /events`, `GET /events/:id`, `GET /events/:id/why`, `GET /projects`, `GET /projects/:p/events?limit=&before_seq=&artifact_id=&actor_id=&since=&text=` (a `{events, truncated, next_before_seq}` page of the **newest** matches; default `limit` 100), `GET /projects/:p/head`, `GET /projects/:p/verify`, `GET /projects/:p/status`.
...
On the git side `session`, `ide`, `workspace` and `surface` describe **the process that produced the commit**, so only the live post-commit/post-merge hook stamps
...
**Not next:** C2PA / line attribution, LangSmith / tracing, an IdP, TRACE/TEE, AI-BOM, a compliance-deadline pitch, adding Cursor to `retrace-admin` defaults / treating it as a sixth product harness, a fourth trailer vocabulary, Claude-Code-only managed hooks as the completeness strategy, more adapters beyond the explicitly experimental OpenClaw audit pilot, batch-amending the ledger. Signed checkpoints / Rekor are a later export of completeness, not a reason to pause the gate. This checkout dogfoods a pinned `cursor-agent` identity (Worker credential + git-hook allow-list; token off-repo).
