...
- **B — GitHub vs hook** — **BUILT 2026-09-01**, awaiting two config steps. What changed: push events get their own key `gh:push:<repo>:<sha>` (the old `git:<sha>` collapsed both producers into one event); actor resolution moved to core `commit-actor.ts` so the webhook and the hook resolve the same actor from the same message (the old mapping stamped the git author as a human, so every agent commit would have "disagreed"); reconcile adds `producer_disagreement` — fail when hook and stamped webhook name different actors, warn when only one producer sealed a commit after both were active, and a push-shaped event without the server's `webhook:github` stamp seals nothing. Also fixed on the way: the gate's shallow checkout made HEAD's diff the whole repo (now fails closed; `fetch-depth: 2`). Switched on 2026-09-01 (webhook 671790737 + Worker a73fea86): commits are sealed twice from `3d8539a` on.

*Hardened after Codex's review of `1491c67` (four findings + Jordan's refinement, all accepted): (1) a hook seal is the **exact, configured** server stamp `assert:<this repo's hook credential>` plus git-adapter shape — never a credential class (negative test: the Drive forwarder's assert credential with a perfect git payload is a claim, not a seal); `owner` is an opt-in operator override, documented as not an independent witness; unstamped legacy seals need `--allow-unstamped-seals`; a claim is reported as `missing_commit` naming its stamp. (2) Dual witness is enforced: an agent commit sealed by one producer once both are active **fails** (`dual_witness: warn` for unpushed local runs); the abandoned original of an amend/rebase is `unreachable_seal` — the CLI checks every sealed sha against git (`cat-file --batch-check`) and vanished seals are excluded from window boundaries so the replacement's edits still reconcile. (3) Window boundaries are one per sha at the hook's seq, so the webhook's later copy cannot swallow an edit. (4) An explicit artifact role is authoritative; the verb only decides for role-less refs.*

*Hardened once more after Codex's review of `05c61f9`: `unreachable_seal` uses ref reachability (`rev-list --all`), not object existence — an amended original that lingers as a loose object is now detected (regression test on a real temp repo); a horizon (the oldest reachable seal's seq) means a shallow clone's unfetched history keeps bounding windows instead of opening them (regression test on a real `--depth 2` clone: a stale edit cannot cover an unlogged HEAD change); the gate checks out full history. Also: legacy commit events sealed before the server began stamping are window boundaries but never trusted seals, while an unstamped commit event after stamping began is a claim that bounds nothing (so a planted "commit" cannot hide a swept edit). Agent acknowledgements fail closed: humans always may; an agent only when listed in `reconcile.ack_actors` and not sharing the accused seal's model or session — live demonstration #1227 (cursor-agent on grok-4.6 acknowledging a grok commit) is the regression test. Friction item closed 2026-09-01: the checkpoint PR is opened by the `retrace-checkpoint` GitHub App (workflow guard fails closed without its credentials; GITHUB_TOKEN reduced to read-only), so the gate runs on it without "Approve and run". The gate never infers leniency from the ref layout (a detached pull_request checkout has no remote-tracking branch containing HEAD, so `branch -r --contains` proves nothing): `--gate` fails on a lone producer unconditionally; unpushed local work uses the explicit `--local` flag, refused under `GITHUB_ACTIONS`/`CI` (regression test: detached pushed sha, no refs/remotes branch contains HEAD). The checkpoint workflow's PR body is now built in a file — the earlier inline body broke the YAML, so every push since 05:43 UTC produced a jobless failed run and the 06:17 schedule never fired.*
